...
import "../Css/CardProducto.css"; // Importamos el archivo CSS

const CardProducto = ({ id, name, price, detail, img }) => {
  const { addToCart } = useCart();
  const [añadido, setAñadido] = useState(false);

  const handleAddToCart = () => {
    addToCart({ id, name, price, detail, img });
    setAñadido(true);
          ...
          <div className="col-md-8 h-100 border-start">
            <div className="card-body d-flex flex-column p-4 h-100">
              ...
              <div className="flex-grow-1 product-specs-container">
                <p className="fw-bold text-uppercase product-specs-label">Descripción:</p>
                <div className="d-flex flex-wrap gap-1">
                  {detail.map((item, index) => (
                    <span key={index} className="badge bg-secondary-subtle text-dark spec-badge">
                      • {item}
                    </span>
                  ))}
                </div>
